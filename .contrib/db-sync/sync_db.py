...
def repository_from_remote(remote_url: str) -> Optional[str]:
    patterns = (
...
def candidate_repositories(root: Path, requested: Optional[str]) -> List[str]:
    repositories: List[str] = []

    def add(repository: Optional[str]) -> None:
...
def find_run(root: Path, repository: str, commit: str) -> Optional[int]:
    result = run(
        [
            "gh",
            "run",
            ...
            for member in bundle.infolist():
                ...
                try:
                    contained = os.path.commonpath(
                        [str(destination_root), str(target)]
